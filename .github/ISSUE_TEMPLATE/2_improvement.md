...
<!--
      ...
      share -> boot -> warmup pipeline, the EXL3 packed-mul1 overlay or the E3 v2
      grouped fat-expert kernels, the file-backed Engram row store and ./start.sh
      pack, the memory guards and budget, long-prefill chunking and the sparse
      indexer, DSpark (in-checkpoint draft) configuration and acceptance
      measurement, the two-node TP=2 orchestration, and so on.
-->
