"""Create an exclusive opt-in overlay for the exact validated source/binary pair.
...
ADAPTER_SHA = 'b8688b9b9c88fbca58315e9bb632f6a241b937904534a03cbff1360fbf6980fe'
...
def make_profile(stock, artifacts, runtime_directory, output):
    ...
    checked(artifacts / 'goal50-fixed-coop.so', BINARY_SHA)
    checked(artifacts / 'goal50_fixed_coop_runtime.py', ADAPTER_SHA)
    runtime_root = PurePosixPath(runtime_directory)
    if not runtime_root.is_absolute() or '..' in runtime_root.parts:
        raise ValueError('Use an absolute container runtime directory without parent traversal')
    footer = ('\n# Explicit fixed cooperative MoE opt-in; unsupported calls stay stock.\n'
              'import runpy as _coop_runpy\nimport sys as _coop_sys\n'
              f'_coop_setup = _coop_runpy.run_path({str(runtime_root / "goal50_fixed_coop_runtime.py")!r})\n'
