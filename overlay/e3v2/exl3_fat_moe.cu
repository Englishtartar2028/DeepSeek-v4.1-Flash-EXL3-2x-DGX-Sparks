...
// Grouped fat-expert kernels, v2. See exl3_fat_moe.cuh for the contract.
//
// Mainloop: TILE_M rows x 128 columns per CTA, K advanced 32 per pipeline
// stage with cp.async multi-buffering. B tiles (16x16 trellis tiles, bits*16
// int16 words each) are staged through shared memory and dequantized in
...
// streams (gate, up) over two A streams (h13g, h13u) for the same 128
// intermediate columns so the SwiGLU and the down-projection input Hadamard
// fuse into its epilogue. The down kernel runs two B streams (adjacent
// 128-column halves) over one A stream.
...
// ---------------------------------------------------------------------------
// Shared mainloop: acc[NS][MB][2] += A_(NA==2 ? s : 0)(tile rows, K) @ B_s(K, 128 cols)
// ---------------------------------------------------------------------------
...
// NA: number of A (activation) buffers; 2 = one per B stream (gate/up with
// their own input sign vectors), 1 = shared.
template <int MB, int NS, int NB_STRIDE, int BITS, int CB, int NA>
__device__ __forceinline__ void fm_mainloop(
    const half* const (&a)[NA],          // fat-row buffers [rows_cap, size_k]
    int size_k,
    int row0,
    int rows,
    const uint16_t* const (&packed)[NS], // per-stream trellis, (K/16, tiles_n, bits*16)
    int tiles_n,
    int n_block0,                        // first 16-col block of this tile
    half* sh_a,                          // FM_STAGES * NA * MB*16*32 halves
    uint16_t* sh_b,                      // FM_STAGES * NS * b_stage_words
    FragC (&acc)[NS][MB][2])
{
    constexpr int TILE_M = MB * 16;
    constexpr int A_TILE = TILE_M * FM_TILE_K;           // halves, one A buffer
    constexpr int A_STAGE = NA * A_TILE;                 // halves per stage
    constexpr int A_CHUNKS = NA * TILE_M * 4;            // 16 B chunks per stage
    constexpr int A_ITERS = (A_CHUNKS + FM_THREADS - 1) / FM_THREADS;
    constexpr int PW = FmPack<BITS>::words;
    constexpr int CH = FmPack<BITS>::chunks;
    constexpr int B_STAGE = NS * FmPack<BITS>::b_stage_words;   // int16 words
    constexpr int B_CHUNKS = NS * 2 * FM_WARPS * CH;            // 16 B chunks per stage
    ...
    auto load_stage = [&](int stage, int kt)
    {
        ...
        for (int i = 0; i < A_ITERS; ++i)
        {
            ...
            if (c < A_CHUNKS)
            {
                ...
                const half* src = a[buf] + (int64_t) (row0 + src_row) * size_k + kt * FM_TILE_K + chunk * 8;
                half* dst = sa + buf * A_TILE + row * FM_TILE_K + fm_swz(row, chunk) * 8;
                cp_async(dst, src);
            }
        }
        ...
        for (int i = 0; i < B_ITERS; ++i)
        {
            ...
            if (c < B_CHUNKS)
            {
                int s = c / (2 * FM_WARPS * CH);
                int r = c % (2 * FM_WARPS * CH);
                int j = r / (FM_WARPS * CH);         // k16 sub-step
                int nb = (r / CH) % FM_WARPS;        // 16-col block (= warp)
                int q = r % CH;                      // 16 B chunk of the tile
                const uint16_t* src = packed[s]
                    + ((int64_t) (kt * 2 + j) * tiles_n + n_block0 + s * NB_STRIDE + nb) * PW + q * 8;
                uint16_t* dst = sb + (s * 2 + j) * (FM_WARPS * PW) + nb * PW + q * 8;
                cp_async(dst, src);
            }
        }
    };
        ...
        for (int j = 0; j < 2; ++j)
        {
            ...
            for (int s = 0; s < NS; ++s)
            {
                const uint32_t* wb = reinterpret_cast<const uint32_t*>(
                    sb + (s * 2 + j) * (FM_WARPS * PW) + warp * PW);
                dq_dispatch<BITS, CB>(wb, lane << 3, fb[s][0], fb[s][1]);
            }
            #pragma unroll
            for (int mb = 0; mb < MB; ++mb)
            {
                ...
                #pragma unroll
                for (int i = 0; i < NA; ++i)
                    ldsm4(fa[i], sa + i * A_TILE + row * FM_TILE_K + fm_swz(row, chunk) * 8);
                #pragma unroll
                for (int s = 0; s < NS; ++s)
                {
                    constexpr int dummy = 0; (void) dummy;
                    const FragA& fas = fa[NA == 2 ? s : 0];
                    ptx_mma_m16n8k16(fas, fb[s][0], acc[s][mb][0]);
                    ptx_mma_m16n8k16(fas, fb[s][1], acc[s][mb][1]);
                }
            }
        }
    }
    ...
    float act_limit)
{
    ...
    uint16_t* sh_b = reinterpret_cast<uint16_t*>(sh_a + FM_STAGES * NA * MB * 16 * FM_TILE_K);
    ...
    const half* const a[NA] = { h13g, h13u };
    ...
    for (int seg = blockIdx.y; seg < num_segs; seg += gridDim.y)
    {
        ...
        FragC acc[NS][MB][2];
        fm_mainloop<MB, NS, 0, BITS, CB, NA>(a, size_k, row0, rows, packed, tiles_n, n_base / 16, sh_a, sh_b, acc);
    ...
    int size_n)
{
    ...
    for (int seg = blockIdx.y; seg < num_segs; seg += gridDim.y)
    {
        ...
        FragC acc[NS][MB][2];
        fm_mainloop<MB, NS, FM_TILE_N / 16, BITS, CB, NA>(a, size_k, row0, rows, packed, tiles_n, n_base / 16, sh_a, sh_b, acc);
...
template <int BITS, int CB>
void launch_gateup(
    const at::Tensor& h13g, const at::Tensor& h13u,
    const at::Tensor& gate_ptrs, const at::Tensor& up_ptrs,
    const at::Tensor& gate_svh_ptrs, const at::Tensor& up_svh_ptrs, const at::Tensor& down_suh_ptrs,
    at::Tensor& h2,
    const at::Tensor& seg_expert, const at::Tensor& seg_row0, const at::Tensor& seg_rows, const at::Tensor& num_segs,
    float act_limit, int size_k, int size_n, cudaStream_t stream)
{
    constexpr int smem = fm_smem_bytes<FM_MB_GATEUP, 2, BITS, 2>();
    static bool attr_set = false;
    if (!attr_set)
    {
        cudaFuncSetAttribute(fm_gateup_kernel<BITS, CB>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
        attr_set = true;
    }
    dim3 grid(size_n / FM_TILE_N, fm_grid_y(seg_expert.size(0)));
    fm_gateup_kernel<BITS, CB><<<grid, FM_THREADS, smem, stream>>>(
        reinterpret_cast<const half*>(h13g.data_ptr()),
        reinterpret_cast<const half*>(h13u.data_ptr()),
...
template <int BITS, int CB>
void launch_down(
    const at::Tensor& h2, const at::Tensor& down_ptrs, const at::Tensor& down_svh_ptrs, at::Tensor& out,
    const at::Tensor& row_token, const at::Tensor& row_weight,
    const at::Tensor& seg_expert, const at::Tensor& seg_row0, const at::Tensor& seg_rows, const at::Tensor& num_segs,
    int size_k, int size_n, cudaStream_t stream)
{
    constexpr int smem = fm_smem_bytes<FM_MB_DOWN, 2, BITS, 1>();
    static bool attr_set = false;
    if (!attr_set)
    {
        cudaFuncSetAttribute(fm_down_kernel<BITS, CB>, cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
        attr_set = true;
    }
    dim3 grid(size_n / (2 * FM_TILE_N), fm_grid_y(seg_expert.size(0)));
    fm_down_kernel<BITS, CB><<<grid, FM_THREADS, smem, stream>>>(
...
bool fm_supported(int64_t bits, int64_t cb)
{
    return (bits == 4 && (cb == 1 || cb == 2)) || ((bits == 3 || bits == 2) && cb == 2);
}
...
int64_t exl3_fat_moe_abi() { return 2; }
...
void exl3_fat_moe_gateup(
    ...
    int64_t cb)
{
    const at::cuda::OptionalCUDAGuard device_guard(h13g.device());
    cudaStream_t stream = at::cuda::getCurrentCUDAStream().stream();
    TORCH_CHECK(fm_supported(bits, cb), "exl3_fat_moe: unsupported (bits, codebook) = (", bits, ", ", cb,
                "); compiled for (4,mcg) (4,mul1) (3,mul1) (2,mul1)");
    for (const at::Tensor* h : { &h13g, &h13u })
        TORCH_CHECK(h->is_cuda() && h->is_contiguous() && h->scalar_type() == at::kHalf && h->dim() == 2,
                    "h13g/h13u must be contiguous half [rows_cap, K] CUDA tensors");
    TORCH_CHECK(h13u.size(0) == h13g.size(0) && h13u.size(1) == h13g.size(1), "h13g / h13u shape mismatch");
    ...
    TORCH_CHECK(h2.size(0) == h13g.size(0), "h2 / h13 row capacity mismatch");
    int size_k = (int) h13g.size(1);
    ...
    if (bits == 4 && cb == 1)
        launch_gateup<4, 1>(h13g, h13u, gate_ptrs, up_ptrs, gate_svh_ptrs, up_svh_ptrs, down_suh_ptrs, h2, seg_expert, seg_row0, seg_rows, num_segs, lim, size_k, size_n, stream);
    else if (bits == 4 && cb == 2)
        launch_gateup<4, 2>(h13g, h13u, gate_ptrs, up_ptrs, gate_svh_ptrs, up_svh_ptrs, down_suh_ptrs, h2, seg_expert, seg_row0, seg_rows, num_segs, lim, size_k, size_n, stream);
    else if (bits == 3 && cb == 2)
        launch_gateup<3, 2>(h13g, h13u, gate_ptrs, up_ptrs, gate_svh_ptrs, up_svh_ptrs, down_suh_ptrs, h2, seg_expert, seg_row0, seg_rows, num_segs, lim, size_k, size_n, stream);
    else
        launch_gateup<2, 2>(h13g, h13u, gate_ptrs, up_ptrs, gate_svh_ptrs, up_svh_ptrs, down_suh_ptrs, h2, seg_expert, seg_row0, seg_rows, num_segs, lim, size_k, size_n, stream);
    cuda_check(cudaPeekAtLastError());
}
    ...
    int64_t cb)
{
    ...
    if (bits == 4 && cb == 1)
        launch_down<4, 1>(h2, down_ptrs, down_svh_ptrs, out, row_token, row_weight, seg_expert, seg_row0, seg_rows, num_segs, size_k, size_n, stream);
    else if (bits == 4 && cb == 2)
        launch_down<4, 2>(h2, down_ptrs, down_svh_ptrs, out, row_token, row_weight, seg_expert, seg_row0, seg_rows, num_segs, size_k, size_n, stream);
    else if (bits == 3 && cb == 2)
        launch_down<3, 2>(h2, down_ptrs, down_svh_ptrs, out, row_token, row_weight, seg_expert, seg_row0, seg_rows, num_segs, size_k, size_n, stream);
    else
        launch_down<2, 2>(h2, down_ptrs, down_svh_ptrs, out, row_token, row_weight, seg_expert, seg_row0, seg_rows, num_segs, size_k, size_n, stream);
    cuda_check(cudaPeekAtLastError());
}
